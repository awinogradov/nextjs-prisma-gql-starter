...
type CallbackSetter<T> = (handler: T) => void;

type HandlerPair<T> = readonly [handler: MutableRefObject<T>, setHandler: CallbackSetter<T>];
 ...
 */
export const useHandlerSetterRef = <T extends (...args: any[]) => any>(handler?: T): HandlerPair<T> => {
